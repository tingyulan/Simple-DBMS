...
/*
    printf("select_fields_len: %ld\n", fields_len);
    printf("select_fields:");
    printf("%s\n", argument);
*/
...
int add_where_field(Command_t *cmd, const char *argument) {
    // printf("IN WHERE FIELD\n");
    // printf("arg: %s\n", argument);
    size_t where_fields_len = cmd->cmd_args.sel_args.where_fields_len;

    //int count=0;
    char **buf = (char**)malloc(sizeof(char*) * (where_fields_len+1));
    if (buf == NULL) {
        return 0;
    }

    if (cmd->cmd_args.sel_args.where_fields) {
        memcpy(buf, cmd->cmd_args.sel_args.where_fields, sizeof(char*) * where_fields_len);
        free(cmd->cmd_args.sel_args.where_fields);
    }

    cmd->cmd_args.sel_args.where_fields = buf;
    cmd->cmd_args.sel_args.where_fields[where_fields_len] = strdup(argument);
    cmd->cmd_args.sel_args.where_fields_len++;
    return 1;
}

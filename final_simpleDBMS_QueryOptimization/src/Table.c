...
int delete_User(Table_t *table, int idx) {
    //size_t table_len;
    int front, back;
    if (!table) {
        return 0;
    ...
    memcpy(new_user_buf, table->users, sizeof(User_t)*front);
    memcpy((new_user_buf)+front, (table->users)+front+1, sizeof(User_t)*back);
